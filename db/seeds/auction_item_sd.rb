...
products = ::Product.joins(:category).where(id: product_companies.pluck(:product_id)).select(:id, :name, :category_id, 'categories.name as category_name')
...
50.times.map do |time|
  ...
  record_attributes.push({
    ...
    year_of_manufacture: Faker::Date.between(from: 10.years.ago, to: Date.today),
    created_at: now,
    updated_at: now,
  })
end
